...
<div class="flex-between-start" style="background-color: #5D5FEF; height: 140px;">
    <div class="flex-c-center-start h-full">
      <div class="m-x-20">
          <span style="font-weight: bold; font-size: 20px; color: #FFFFFF;">热力等级</span>
      </div>
      <div class="m-x-20 m-t-4">
        <span style="font-size: 14px; color: #FFFFFF;">主键、等级名称、颜色。</span>
      </div>
    </div>
...
<el-tab-pane
:label=" $t('page.view')  +' 订单建议'"
>
<order_suggestion_component
 :params="{heat_level_enum_heat_level_enum_id_1:routerQuery.heat_level_enum_id}"
 />
</el-tab-pane>
</el-tabs>
</base-wrapper>
  ...
  proxy.$refs.dataFormRef.validate(async (valid) => {
    if (valid) {
      ...
      if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
          ...
          let res = await proxy.$api.heat_level_enum['update'](form.value);
          proxy.$modal.msgSuccess(res.message);
          if(res.code == 0){
            refresh();
          }
...
async function getDetail(id) {
    if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
        ...
        let res = await proxy.$api.heat_level_enum['get'](id);
        if(res.code == 0){
            form.value = Object.assign({}, res.data);
        }
        else{
...
function refresh(){
    if(routerQuery.heat_level_enum_id){
        getDetail(routerQuery.heat_level_enum_id);
    }
    if(detail.value == 'all'){
        if(routerQuery.heat_level_enum_id){
            getDetailAll(routerQuery.heat_level_enum_id);
        }
    }
}
