    ...
    <div id="sidebar" class="flex-c-between-start menu p-r-2">
        ...
        >
          ...
          <div v-if="model === 'PREVIEW'">
            <el-menu-item index="/api_setting_manger">
              <template #title>{{ $t('menu.api') }}</template>
            </el-menu-item>
          </div>

          <div>
            <el-menu-item index="/system_setting_manger">
          ...
          <div>
            <el-menu-item index="/login_manger">
              <template #title>{{ $t('menu.auth') }}</template>
            </el-menu-item>
          </div>
      ...
      <div class="w-full m-b-30">
        ...
        <div class="flex-end-center m-r-20">
          ...
          <el-button round class="m-l-20" @click="logout">{{$t('login.logout_btn')}}</el-button>
        </div>
      </div>
...
<script setup>
  ...
  function openHelp() {
    window.open('https://vvx03gck2p.feishu.cn/wiki/HlrAwPiedisWKokRhlvcBTAynpf', '_blank');
  }
