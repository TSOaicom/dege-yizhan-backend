...
firstSearchComment="状态"
:searchData="listQuery"
@refreshTableData="refreshTableData"
@addEvent="handleAdd"
uploadExcelAPI="cert_status_enum.import"
table_name="cert_status_enum"
firstSearchData="status"
>
<template #collapse>
    <el-form-item  label="状态" prop="status">
        <el-input clearable placeholder="状态" v-model="listQuery.status" />
    </el-form-item>
...
:query="{cert_status_enum_id: scope.row.cert_status_enum_id}"
text="详情"
>
</base-info-btn>
<base-edit-btn
...
function handleSelectionChange(selection) {
    multipleSelection.value = selection.map(item => item.cert_status_enum_id);
}
...
async function handleDelete(row) {
  if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
    let res = await proxy.$api.table.delete({
        table_name: 'cert_status_enum',
        param: {
            cert_status_enum_id: row.cert_status_enum_id
        }
    });
    proxy.$modal.msgSuccess(res.message);
  }
  else{
    let res = await proxy.$api.cert_status_enum.delete(row.cert_status_enum_id);
    proxy.$modal.msgSuccess(res.message);
  }
  refreshTableData();
}
...
async function handleBatchDelete() {
    ...
    res = await proxy.$api.table.deleteBatch({
        ...
        res = await proxy.$api.cert_status_enum.deleteBatch(ids);
    }
...
async function handleExport() {
  try {
    await proxy.$api.cert_status_enum.downloadExcel({
      ...listQuery.value
      ...
      proxy.$refs.dataFormRef.validate(async (valid) => {
        if (valid) {
          if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
            ...
            let res = await proxy.$api.cert_status_enum[dialogStatus.value](form.value);
            proxy.$modal.msgSuccess(res.message);
          }
          refreshTableData();
          dialogVisible.value = false;
