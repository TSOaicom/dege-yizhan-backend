...
firstSearchComment="反馈内容"
:searchData="listQuery"
@refreshTableData="refreshTableData"
@addEvent="handleAdd"
uploadExcelAPI="feedback.import"
table_name="feedback"
...
<template #collapse>
    ...
    <el-form-item  label="反馈内容" prop="content">
        <el-input clearable placeholder="反馈内容" v-model="listQuery.content" />
    </el-form-item>
...
:query="{feedback_id: scope.row.feedback_id}"
text="详情"
>
</base-info-btn>
<base-edit-btn
...
function handleSelectionChange(selection) {
    multipleSelection.value = selection.map(item => item.feedback_id);
}
...
async function handleDelete(row) {
  if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
    ...
    let res = await proxy.$api.feedback.delete(row.feedback_id);
    proxy.$modal.msgSuccess(res.message);
  }
  refreshTableData();
}
...
async function handleBatchDelete() {
    ...
    res = await proxy.$api.table.deleteBatch({
        ...
        res = await proxy.$api.feedback.deleteBatch(ids);
    }
...
async function handleExport() {
  try {
    await proxy.$api.feedback.downloadExcel({
      ...listQuery.value
      ...
      proxy.$refs.dataFormRef.validate(async (valid) => {
        if (valid) {
          if(import.meta.env.VITE_APP_MODEL === 'PREVIEW'){
            ...
            let res = await proxy.$api.feedback[dialogStatus.value](form.value);
            proxy.$modal.msgSuccess(res.message);
          }
          refreshTableData();
          dialogVisible.value = false;
