<template>
  ...
  <base-login-weapp v-else-if="login_type === 'weapp'" :relevanceTable="relevanceTable" @loginSuccess="login_success" @loginFail="login_error" @service="service_click" @privacy="privacy_click" />
  ...
  <base-login-oauth2-google v-else-if="login_type === 'google'" :relevanceTable="relevanceTable" @loginSuccess="login_success" @loginFail="login_error"/>


  <view v-if="login_type === 'passwd' || login_type === 'mail' || login_type === 'wechat' || login_type === 'phone' || login_type === 'google'"
...
onLoad(async (options) => {
  ...
  try {
    ...
    let res = await proxy.$app.globals.read({
      variableName: "h5_token",
    })
    if (res.success) {
      let res = await proxy.$app.atuh.getUser()
