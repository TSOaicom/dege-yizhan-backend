  ...
  <view v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <view class="bg-white rounded-2xl w-4/5 max-w-md p-4 shadow-lg">
      <view class="text-xl font-bold text-center mb-4 text-gray-800">
        {{ t('pay.price_title') }}：<text class="text-red-500">￥{{ price }}</text>
      </view>

      <view class="mb-4 min-h-[200px]">
        <text v-if="tipShow" class="text-gray-700 font-medium mb-2 block bg-gray-200 rounded-lg p-3">
          {{ t('pay.pay_tip') }}
          <uni-badge text="?" type="info" @click="jump_help"/>
        </text>
        <radio-group @change="onRadioChange">
          <view class="space-y-2">
            <label
                v-for="method in payMethods.filter(m => m.enable)"
                :key="method.value"
                class="flex items-center justify-between p-3 rounded-lg border"
                :class="selectedMethod === method.value ? 'border-primary-500 bg-primary-50' : 'border-gray-300'"
            >
              <view v-if="method.enable" class="flex items-center justify-between w-full">
                <text class="text-gray-700">{{ method.label }}</text>
                <radio class="text-primary-500 transform scale-110"
                    :value="method.value"
...
const payMethods = ref([
  {label: proxy.$tt('pay.pay_mock'), value: 'mock', enable: true},
  {label: proxy.$tt('pay.pay_wechat'), value: 'wechat', enable: true},
  {label: proxy.$tt('pay.pay_alipay'), value: 'alipay', enable: true},
  {label: proxy.$tt('pay.pay_stripe'), value: 'stripe', enable: true}
])

onLoad(async (options) => {
  let res = await proxy.$app.payment.payMethod()
  if (!res.success) {
    return
  }
  ...
  if (res.data.includes('stripe')) {
    payMethods.value[3].enable = true;
  } else {
    payMethods.value[3].enable = false;
  }

  if (res.data.includes('alipay')) {
    payMethods.value[2].enable = true;
  } else {
    payMethods.value[2].enable = false;
  }

  if (options.stripe_callback != null && options.stripe_callback !== '') {
    console.log("stripe_callback", options.stripe_callback)
    emit('paySuccess', {
          orderNo: options.stripe_callback
    }); 
  }
})
...
async function handlePay() {
  ...
  if (pages && pages.length > 0) {
    ...
    currentUrl = currentPage.route || currentPage.$page?.meta?.isEntry ? '/' : currentPage.route
  }
  //#endif
  ...
  let res = await proxy.$app.payment.createOrders(request_body)
  console.log(res,"res====")
  if (!res.success) {
    console.log(res.message,"打印")
    proxy.$app.ui.toast().message(res.message ? res.message:'支付失败,请重试').icon('none')
    hide();
    return
  }
